...
export default function PlannerPage() {
  ...
  const saveTasks = (updated) => { localStorage.setItem(TASKS_KEY, JSON.stringify(updated)); };
  ...
  const toggleDone = (id) => { const u = tasks.map(tk => tk.id === id ? { ...tk, done: !tk.done } : tk); setTasks(u); saveTasks(u); };
  const removeTask = (id) => { const u = tasks.filter(tk => tk.id !== id); setTasks(u); saveTasks(u); };
  const addTask = () => {
    if (!newTask.title.trim()) return;
    const u = [...tasks, { ...newTask, id: Date.now(), done: false, color: { deadline: "red", study: "teal", session: "amber", quiz: "green", other: "primary" }[newTask.type] }];
    setTasks(u); saveTasks(u); setNewTask({ title: "", type: "study", date: selectedDate }); setShowAdd(false);
  };
  ...
  const mokuMsg = incompleteTodayCount > 0
    ? `Kamu punya ${incompleteTodayCount} task hari ini. Yuk mulai! 💪`
    : upcomingDeadlines.length > 0
    ? `Deadline terdekat: "${upcomingDeadlines[0].title}" — ${daysUntil(upcomingDeadlines[0].date)} 🧘`
    : "Gak ada task pending. Santai atau tambahin rencana? ✨";

  const getAiTip = useCallback(async () => {
    setTipLoading(true); setAiTip("");
    const deadlineInfo = upcomingDeadlines.length > 0
      ? `Deadline terdekat: "${upcomingDeadlines[0].title}" pada ${upcomingDeadlines[0].date}.`
      : "Tidak ada deadline mendesak.";
    const todayTasks = tasksForDate(todayStr);
    const taskInfo = todayTasks.length > 0
      ? `Task hari ini: ${todayTasks.map(tk => `"${tk.title}" (${tk.type}${tk.done ? ", done" : ""})`).join(", ")}.`
      : "Tidak ada task hari ini.";
    try {
      const res = await callGemini(
        "Kamu adalah asisten belajar bernama Moku. Berikan 1 tips belajar singkat (max 2 kalimat) dalam Bahasa Indonesia yang relevan dengan kondisi jadwal user. Fokus ke produktivitas, motivasi, atau strategi belajar. Jangan lebay, buat natural dan friendly.",
        `${deadlineInfo} ${taskInfo} Berikan tips yang relevan untuk hari ini.`
      );
      setAiTip(res);
    } catch {
      setAiTip("Konsistensi kecil setiap hari lebih baik dari belajar maraton sekali. Keep going! 💪");
    }
    setTipLoading(false);
  }, [tasks, todayStr]);

  useEffect(() => {
    const cached = sessionStorage.getItem("planner-ai-tip");
    if (cached) { setAiTip(cached); return; }
    getAiTip().then(() => {
      if (aiTip) sessionStorage.setItem("planner-ai-tip", aiTip);
    });
  ...
  return (
    <div style={{ padding: "20px 16px", maxWidth: 800, margin: "0 auto" }}>
      <style>{`@keyframes fadeIn{from{opacity:0;transform:translateY(8px)}to{opacity:1;transform:translateY(0)}}@keyframes shimmer{0%{opacity:.5}50%{opacity:1}100%{opacity:.5}}`}</style>
      ...
      {/* Moku msg */}
      <div style={{ background: t.primaryBg, border: `1.5px solid ${t.primary}20`, borderRadius: 16, padding: "12px 16px", marginBottom: 12, display: "flex", gap: 10, alignItems: "center" }}>
        <span style={{ fontSize: 22 }}>🧠</span>
        <div style={{ fontSize: 12, color: t.sub, lineHeight: 1.6 }}>{mokuMsg}</div>
      </div>

      {/* AI Tip */}
      <div style={{ background: t.tealBg || t.primaryBg, border: `1.5px solid ${t.teal}25`, borderRadius: 16, padding: "12px 16px", marginBottom: 16, display: "flex", gap: 10, alignItems: "flex-start" }}>
        <span style={{ fontSize: 20, flexShrink: 0 }}>✨</span>
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 10, fontWeight: 800, color: t.teal, textTransform: "uppercase", letterSpacing: 1, marginBottom: 4 }}>Tips AI Hari Ini</div>
          {tipLoading ? (
            <div style={{ fontSize: 12, color: t.dim, animation: "shimmer 1.5s ease-in-out infinite" }}>Moku lagi mikir tips terbaik buat kamu...</div>
          ) : (
            <div style={{ fontSize: 12, color: t.sub, lineHeight: 1.6 }}>{aiTip || "—"}</div>
          )}
        </div>
        <button onClick={getAiTip} disabled={tipLoading} title="Refresh tip" style={{ background: "none", border: `1px solid ${t.teal}30`, borderRadius: 8, padding: "4px 8px", fontSize: 11, color: t.teal, cursor: tipLoading ? "not-allowed" : "pointer", flexShrink: 0 }}>🔄</button>
      </div>
      ...
      <div style={{ background: t.card, border: `1.5px solid ${t.border}`, borderRadius: 18, padding: "16px 18px", marginBottom: 16, boxShadow: t.shadow }}>
        ...
        {selectedTasks.length === 0 ? (
          <div style={{ textAlign: "center", padding: "20px 0", color: t.dim, fontSize: 13 }}>Belum ada task. Klik "+ Task" untuk tambah.</div>
        ) : (
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.4)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200 }} onClick={() => setShowAdd(false)}>
          <div onClick={e => e.stopPropagation()} style={{ background: t.card, borderRadius: 22, padding: 24, width: 360, boxShadow: t.shadow, animation: "fadeIn .2s ease" }}>
            ...
            <div style={{ marginBottom: 12 }}>
              <label style={{ fontSize: 10, fontWeight: 700, color: t.dim, display: "block", marginBottom: 4, textTransform: "uppercase", letterSpacing: 1 }}>Judul</label>
              <input value={newTask.title} onChange={e => setNewTask(p => ({ ...p, title: e.target.value }))} onKeyDown={e => e.key === "Enter" && addTask()} style={{ width: "100%", padding: "10px 14px", borderRadius: 10, border: `1.5px solid ${t.inputB}`, background: t.input, color: t.text, fontSize: 13, outline: "none", fontFamily: "inherit", boxSizing: "border-box" }} />
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 16 }}>
              <div>
                <label style={{ fontSize: 10, fontWeight: 700, color: t.dim, display: "block", marginBottom: 4, textTransform: "uppercase", letterSpacing: 1 }}>Tipe</label>
                <select value={newTask.type} onChange={e => setNewTask(p => ({ ...p, type: e.target.value }))} style={{ width: "100%", padding: "10px 12px", borderRadius: 10, border: `1.5px solid ${t.inputB}`, background: t.input, color: t.text, fontSize: 12, outline: "none", fontFamily: "inherit" }}>
              ...
              <div>
                <label style={{ fontSize: 10, fontWeight: 700, color: t.dim, display: "block", marginBottom: 4, textTransform: "uppercase", letterSpacing: 1 }}>Tanggal</label>
                <input type="date" value={newTask.date} onChange={e => setNewTask(p => ({ ...p, date: e.target.value }))} style={{ width: "100%", padding: "10px 12px", borderRadius: 10, border: `1.5px solid ${t.inputB}`, background: t.input, color: t.text, fontSize: 12, outline: "none", fontFamily: "inherit" }} />
              </div>
            </div>
